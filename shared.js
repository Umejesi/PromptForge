...
async function callImageAPI(prompt, qualityTier, style, size){
  const {data, error: sessionError} = await sb.auth.getSession();
  const session = data?.session;
  if(sessionError || !session){
    // Force a clean re-auth state so the UI doesn't get stuck half-logged-in
    currentUser = null;
    currentProfile = null;
    updateAuthUI();
  ...
  const res = await fetch(WORKER_URL + 'generate-image', {
    method:'POST', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({ prompt, accessToken: session.access_token, qualityTier, style, size })
  });
  let data2;
  try{
  ...
  if(!res.ok) throw new Error(data2.error || 'Generation failed');
  if(!data2.image) throw new Error('No image was returned — try a different prompt');
  return data2;
}
